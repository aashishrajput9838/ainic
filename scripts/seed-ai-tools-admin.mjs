...
const serviceAccountPath = process.env.FIREBASE_SERVICE_ACCOUNT_PATH || path.resolve('service-account.json');

if (!fs.existsSync(serviceAccountPath)) {
  throw new Error(
    `Service account file not found at ${serviceAccountPath}. ` +
      'Download a service account key JSON from Firebase console and save it there, or set FIREBASE_SERVICE_ACCOUNT_PATH.'
  );
}
...
admin.initializeApp({
  ...
  projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || 'ainic-d525e',
});
...
async function main() {
  console.log(`Seeding ${aiToolsSeed.length} AI tools into Firestore (admin)...`);

  const colRef = db.collection('aiTools');

  for (const tool of aiToolsSeed) {
    try {
      const docRef = await colRef.add(tool);
